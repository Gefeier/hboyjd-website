...
// ====== 车型详情 carousel(2026-06-16 v3) · 无限自走 marquee + 手动 + 暂停 ======
(function initModelsCarousel() {
    const scroll = document.getElementById('modelsScroll');
    if (!scroll) return;
    const prevBtn = document.getElementById('modelsPrev');
    const nextBtn = document.getElementById('modelsNext');
    const bar = document.getElementById('modelsProgressBar');
    if (!prevBtn || !nextBtn || !bar) return;
    ...
    originals.forEach((node) => {
        ...
        clone.classList.add('model-mini-clone');
        // 克隆品如果是链接,去掉 tabindex 防 a11y 重复
        if (clone.tagName === 'A') clone.tabIndex = -1;
        scroll.appendChild(clone);
    });

    function cardStep() {
        const card = scroll.querySelector('.model-mini');
        if (!card) return 340;
        const gap = parseInt(getComputedStyle(scroll).gap) || 20;
    ...
    function updateProgress() {
        const lw = loopWidth();
        if (lw <= 0) return;
        const pct = (scroll.scrollLeft % lw) / lw;
        const visibleRatio = scroll.clientWidth / lw;
        const barWidth = Math.max(15, Math.min(60, visibleRatio * 100));
        bar.style.width = barWidth + '%';
        bar.style.transform = `translateX(${pct * (100 - barWidth) / barWidth * 100}%)`;
    }
    ...
    function checkLoop() {
        const lw = loopWidth();
        if (lw <= 0) return;
        if (scroll.scrollLeft >= lw) {
            scroll.scrollLeft -= lw;
        } else if (scroll.scrollLeft < 0) {
            scroll.scrollLeft += lw;
        }
    }

    // 自走 marquee · 累加器整数化避免浮点丢精度
    const PX_PER_SEC = 35;  // 每秒滚 35 px(慢但能看见动)
    let paused = false;
    let lastTs = 0;
    let driftAccum = 0;
    ...
    function tick(ts) {
        ...
        if (!paused && dt > 0 && dt < 100) {
            ...
            const step = Math.floor(driftAccum);
            if (step >= 1) {
    ...
    scroll.addEventListener('mouseleave', () => { paused = false; lastTs = 0; });
    scroll.addEventListener('touchstart', () => { paused = true; }, { passive: true });
    scroll.addEventListener('touchend', () => {
        // 触摸结束后 2 秒再恢复 marquee,避免抢用户
        setTimeout(() => { paused = false; lastTs = 0; }, 2000);
    });

    // 手动按钮(也算暂停 marquee 一段时间)
    function manualNudge(dir) {
        paused = true;
        scroll.scrollBy({ left: dir * cardStep(), behavior: 'smooth' });
        setTimeout(() => { paused = false; lastTs = 0; }, 3000);
    }
    prevBtn.addEventListener('click', () => manualNudge(-1));
    nextBtn.addEventListener('click', () => manualNudge(1));
    ...
    window.addEventListener('mouseup', () => {
        if (!isDown) return;
        isDown = false;
        setTimeout(() => { paused = false; lastTs = 0; }, 1500);
    });
